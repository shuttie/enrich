...
object Enrich {
  ...
  def run[F[_]: Concurrent: ContextShift: Clock: Parallel: Timer, A](env: Environment[F, A]): Stream[F, Unit] = {
    ...
    val sinkAndCheckpoint: Pipe[F, List[(A, Result)], Unit] =
      _.parEvalMap(env.streamsSettings.concurrency.sink)(chunk =>
        for {
          begin <- Clock[F].realTime(TimeUnit.MILLISECONDS)
          result <- sinkChunk(chunk, env).as(chunk.map(_._1))
          end <- Clock[F].realTime(TimeUnit.MILLISECONDS)
  ...
  def sinkChunk[F[_]: Concurrent: Parallel, A, B](
    ...
    List(
      ...
      env.metrics.enrichLatency(chunk.headOption.flatMap(_._2._2))
    ).parSequence_
  }
      ...
      case Some(sink) =>
        val (bad, serialized) =
          enriched
            ...
            .separate
        sink(serialized) *> Logger[F].error(s"${bad.size} PII events couldn't get sent because they are too big")
      case None =>
        Sync[F].unit
    }
